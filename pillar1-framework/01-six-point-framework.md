...
- **Tier 3 — the multi-archetype stage.** The student holds two or three archetypes at once and looks for their convergence. Our polygon is a *Tier-3-baseline* problem precisely because it is two archetypes (Invariance × Domain Constraints) that must be held together. The six points may compress to four or five *written* movements — adjacent points merge in the prose (often pitfalls folding into the pivot, or connections into the takeaway) — but all six remain present in the analysis, exactly as the density principle above requires; the weight simply shifts onto the pivot and the convergence.
...
### Read first, re-read last

This chapter sits at the front of the book for orientation — you need the grammar before you can watch the other pillars speak it. But it is built to be read **twice**. Return to it after Pillar 5, once the twenty archetypes, the multidirectional moves, and the gems have filled the grammar with content: the same six points that first read as an *imposed format* will then read as the *natural shape* of everything you have learned. Pillar 1 is both the doorway and the capstone — read first for orientation, re-read last as synthesis. The spiral closes here.
